...
for i in {1..30}; do
    if /opt/mssql-tools18/bin/sqlcmd -S localhost -U "$SQL_USER" -P "$SA_PASSWORD" -C -Q "SELECT 1" >/dev/null 2>&1; then
...
for script in /scripts/init/*.sql; do
    [ -f "$script" ] || continue
    echo "Running $script..."
    /opt/mssql-tools18/bin/sqlcmd -S localhost -U "$SQL_USER" -P "$SA_PASSWORD" -C -b -i "$script"
done
